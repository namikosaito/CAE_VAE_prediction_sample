...
def main():
    ...
    alpha = 1  # weight to multiple to prediction
    beta = 1 # weight to multiple to KL
    
    ########## Directory setting ##########
    data_path = "../../data/sample_img_data"
    save_dir = "../result/VAE_" + time.strftime("%Y%m%d_%H%M")
    ...
    height, width, color_ch = train_dataset[0][0].shape

    ########## Setting the model ##########
    torch.manual_seed(1234)
    min_loss = 1000000000000000000000.0

    model = vaemodel.VAE_mid(height, width, color_ch, cnn_ch0, cnn_ch1, cnn_ch2, cnn_ch3, h_dim0, h_dim1, h_dim2, output_dim).to(torch_device)
    ...
    for epoch in range(n_epoch):
        ...
        for data in train_dataloader:
            model.train()
            optimizer.zero_grad()
            concat = data[0]            # image data
            GT_prediction = data[1]     # csv label
            ...
            h, x_hat, prediction, mean, var = model.forward(x, torch_device)
            # print("pred", prediction)
            # print("h", h)
            loss = criterion(x_hat, y)
            loss += criterion(prediction, GT_prediction.to(torch.float32).to(torch_device)) * alpha
            loss -= 0.5 * torch.sum(1 + var - mean **2 - torch.exp(var)) * beta # KL loss
            loss.backward()
            optimizer.step()
